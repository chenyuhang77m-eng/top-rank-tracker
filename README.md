...
热搜榜会过滤时政外交、军事冲突、灾难事故、犯罪案件、疫情医疗负面、时事政治经济新闻等不适合做营销话题的硬新闻；命中后会跳过该条，并继续向后顺延补齐到 50 条。数据中的 `rank` 保留来源榜单原始排名，不重新编号。

热销榜目前抓取 6 个来源，每个榜单最多保留 50 条：
...
前端首页建议仍默认展示每个榜单前 10 条：

```js
list.items.slice(0, 10)
```

单个类目 tab 建议仍显示 10 条，但应先从同一个 `items` 数组的 Top 50 候选池里按类目筛选，再取前 10 条。这样页面上展示的第 10 条，可能对应来源整体榜单的第 40 位或更靠后位置。
