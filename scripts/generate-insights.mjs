...
const categoryRules = {
  ...
  BABY: /母婴|宝宝|婴儿|尿不湿|尿裤|奶粉|奶瓶|奶嘴|童装|童鞋|儿童|玩具|乐器|益智|文具|礼品|学习机|早教|学步|安抚|辅食|月子/,
...
function contentIdeasForRow(row) {
  ...
  const banks = [
    ...
    [/童书|绘本|纸板书|翻翻书|洞洞书|分级读物|亲子阅读|课外阅读|阅读书|书籍|图书/, ["分龄童书囤货清单", "睡前亲子阅读书单实测", "低龄宝宝翻翻书选购指南"]],
...
function benefitForRow(row) {
  ...
  const rules = [
    ...
    [/童书|绘本|纸板书|翻翻书|洞洞书|分级读物|亲子阅读|课外阅读|阅读书|书籍|图书/, "陪伴阅读、分龄启蒙和家庭囤书决策效率"],
...
function categoryFocusLabels(catKey, rows, signals = []) {
  const text = `${rows.map((row) => `${row.sub} ${row.scene} ${(row.hotTerms || []).join(" ")}`).join(" ")} ${signals.map((item) => item.title).join(" ")}`;
  const banks = {
...
function categoryLead(cat, signalSet, rows) {
  ...
  const allSignals = signalSet.all || [...hotSignals, ...shopSignals];
  const labels = categoryFocusLabels(cat.key, rows, allSignals);
  const focus = labels.length ? labels.join("、") : rows.slice(0, 3).map((row) => row.sub).join("、");
  const concern = categoryConcern(cat.key);
  if (hotSignals.length && shopSignals.length) {
    return `今日${cat.name}类同时出现热搜话题和热销商品信号，焦点集中在${focus}；内容端可绑定热点做种草，货架端用热销 SKU 验证卖点和转化承接。`;
  }
  if (shopSignals.length) {
    return `今日${cat.name}类热搜声量不强，但电商热销信号明确，主要集中在${focus}；营销应优先放大商品卖点、真实体验和选购清单，用货架热度反推内容选题。`;
  }
  if (hotSignals.length) {
    return `今日${cat.name}类热搜话题集中在${focus}，用户对${concern}关注度提升，可先绑定热点做场景化内容种草，再观察商品承接。`;
  }
  return `今日${cat.name}类暂无明显热搜或热销信号，内容可聚焦${focus}等常青场景，用具体卖点和选购清单承接搜索需求。`;
}
...
function fallbackRowsForCategory(cat, compact) {
  ...
  const orderedTemplates = [...templates].sort((a, b) => rowScore(b, signalSet) - rowScore(a, signalSet));
...
function normalizeCategory(cat, inputCategory, compact) {
  ...
  for (const row of rowPool) {
    if (sorted.length >= minRowsPerCategory && !sorted.includes(row)) break;
    if (rows.some((existing) => existing.sub === row.sub)) continue;
    rows.push(row);
    if (rows.length >= rowsPerCategory || (sorted.length >= minRowsPerCategory && rows.length >= sorted.length)) break;
  }
  ...
  return {
    ...
    rows: rows.length >= minRowsPerCategory ? rows : fallbackRows.slice(0, rowsPerCategory)
  };
}
...
function categorySchema(rowCount = rowsPerCategory) {
  const minItems = rowCount === 0 ? 0 : minRowsPerCategory;
  const maxItems = rowCount === 0 ? 0 : rowsPerCategory;
  return {
    type: "object",
    additionalProperties: false,
    ...
    properties: {
      ...
      rows: { type: "array", minItems, maxItems, items: rowSchema() }
    }
  };
}
...
function buildSystemPrompt() {
  return [
    ...
    "Workflow: inspect categorySignals.hot and categorySignals.shop together for each category. Hot-search signals explain attention; shopping signals explain demand and conversion. Do not mark a category as weak just because hot-search is thin when shopping signals are strong.",
    "Each category must output 3-5 rows. Generate the subcategories from today's real hot-search and shopping signals first; use fallbackSubcategories only when live signal coverage is not enough.",
    ...
    "Each category lead must summarize both sides when present. If hot-search is weak but shopping is strong, say the category has clear ecommerce/hot-selling signals and explain the product-demand direction; do not write 暂无强热搜信号 as the main conclusion.",
    "Each category object includes lead, bullets, rows. bullets has at most 4 items; rows has 3-5 items."
  ].join("\n");
}
    ...
    messages: [
      ...
      },
      {
        ...
        content: JSON.stringify({
          ...
          requiredTopLevelKeys: ["briefs", "summary", "categories"],
